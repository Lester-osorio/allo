...
class ProductoModel extends Conexion
{
	...
	public function Listar_Stock_010()
	{
		$dbconec = Conexion::Conectar();

		try {
			$query = "CCALL sp_count_productos_stock_bajo();";
			$stmt = $dbconec->prepare($query);
			$stmt->execute();
			$count = $stmt->rowCount();
	...
	public function Insertar_Producto(
		...
		try {
			$query = "CALL sp_insert_producto(:codigo_barra,:nombre_producto,:precio_compra,:precio_venta,
				:precio_venta_mayoreo,:precio_venta_3,:stock,:stock_min,:idcategoria,:idmarca,:idpresentacion,:exento,:inventariable,:perecedero)";
			$stmt = $dbconec->prepare($query);
			...
			$stmt->bindParam(":precio_venta_3", $precio_venta_3);
	...
	public function Editar_Producto(
		...
		$dbconec = Conexion::Conectar();
		try {
			$query = "CALL sp_update_producto(:idproducto,:codigo_barra,:nombre_producto,:precio_compra,:precio_venta,:precio_venta_mayoreo,:precio_venta_3,:stock_min,
