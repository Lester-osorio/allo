...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1) {
            ...
            $this->Cell(105, 10, 'VENTAS (DETALLADAS) AL CONTADO DEL DIA ' . strtoupper($dias[date('w')]) . " " . strtoupper(date('d')) . " DE " . strtoupper(
                $meses[date('n') - 1]
            ) . " DEL " . strtoupper(date('Y')), 0, 0, 'C');
...
$objVenta =  new Venta();
$listado = $objVenta->Listar_Ventas_Detalle('HOY', '', '', 1);
$totales = $objVenta->Listar_Ventas_Totales('HOY', '', '', 1);
