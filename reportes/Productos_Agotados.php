...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1) {
            ...
            $this->Cell(105);
            // Title
            $this->Cell(105, 10, 'REPORTE DE PRODUCTOS PARA REORDEN', 0, 0, 'C');
...
try {
    ...
    $pdf->Cell(22, 5, 'Presentacion', 0, 0, 'L', 1);
    $pdf->Cell(22, 5, 'Costo', 0, 0, 'C', 1);
    $pdf->Cell(22, 5, 'P. Venta 1', 0, 0, 'C', 1);
    $pdf->Cell(22, 5, 'P. Venta 2', 0, 0, 'C', 1);
    $pdf->Cell(22, 5, 'P. Venta 3', 0, 0, 'C', 1);
    ...
    if (is_array($listado) || is_object($listado)) {
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(22, 5, $column["siglas"], 0, 0, 'L', 1);
            $pdf->Cell(22, 5, $column["precio_compra"], 0, 0, 'C', 1);
            $pdf->Cell(22, 5, $column["precio_venta"], 0, 0, 'C', 1);
            $pdf->Cell(22, 5, $column["precio_venta_mayoreo"], 0, 0, 'C', 1);
            $pdf->Cell(22, 5, $column["precio_venta_3"], 0, 0, 'C', 1);
        ...
        $pdf->Text(10, $get_Y + 10, 'TOTAL DE PRODUCTOS: ' . number_format($total, 2, '.', ','));
    }

    $pdf->Output('I', 'Productos_PARA_REORDEN.pdf', true);
} catch (Exception $e) {
